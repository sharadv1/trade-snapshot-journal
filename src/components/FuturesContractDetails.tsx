...
export function FuturesContractDetails({ 
  ...
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 p-3 border rounded-md bg-muted/20">
      ...
      <dd>{actualDetails.tickSize ? Number(actualDetails.tickSize).toFixed(5) : ''}</dd>
      ...
      {actualDetails.tickSize && actualDetails.tickValue && (
        <>
          ...
          <dd className="font-medium">${(Number(actualDetails.tickSize) * Number(actualDetails.tickValue)).toFixed(5)}</dd>
        </>
      )}
